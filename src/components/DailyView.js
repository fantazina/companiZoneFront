...
const DailyView = ({onPage, D_styles ,seq, mainPage}) => {
    ...
    return (
        <div>
            <div className={ D_styles.view_container }>
                {writeDTO && 
                    <div className={ D_styles.view_main }>
                        <div className={ D_styles.top_div }>
                            <div className={ D_styles.back }>
                                <button onClick={ () => onChangePage(0) }>뒤로</button>
                            </div>

                            <h2 className={ D_styles.view_title }>[{ writeDTO.sel }]{ writeDTO.title }</h2> 
                                { writeDTO && window.sessionStorage.getItem('userDTO') && JSON.parse(window.sessionStorage.getItem('userDTO')).id === writeDTO.id && (
                                    <div className={ D_styles.up_button }>
                                        <button onClick={ () => onPage(3) }>수정</button>
                                        <button onClick={ onDeleteSubmit }>삭제</button>
                                    </div>
                                )}
                            <div className={ D_styles.id_list }>{ writeDTO.id } | { getToday(writeDTO.logTime) } | { writeDTO.hit }</div>
                        </div>

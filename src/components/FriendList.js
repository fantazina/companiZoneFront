...
const FriendList = ({ onPage, onSeq, F_styles, pg, onPg, mainOnPage }) => {
    ...
    useEffect(() => {
         axios.get(`https://port-0-companizoneback-ll53u2blrj4us1b.sel5.cloudtype.app/write/getList/3`, { params: { keyword: keyword , pg : pg } })
        .then(res => console.log(res.data))

        axios.get(`https://port-0-companizoneback-ll53u2blrj4us1b.sel5.cloudtype.app/write/getTotal/3`)
        .then(res => {
            let pl = []
    ...
    return (
        <div>
            <div className={ F_styles.upload_main }>
                <h2 style={{ display : 'inline-block' }}>친구 찾기 게시판</h2>
                    <div className={ F_styles.search }>
                    ...
                    <div className={F_styles.friendList}>
                        <div className={ F_styles.list_ }>
                            ...
                            <span className={ F_styles.logTime }>작성일자</span>
                            <span className={ F_styles.hit }>조회수</span>
                        </div>
                    ...
                    <div>
                        <div className={ F_styles.page_num }>{pg > 4  && <span onClick={() => onPg(0 > pg - 10 ? 0 : pg - 10)}>이전</span>}
                            { pageList.filter(fil => pg > 4 ? ( pageList.length - 1 < pg + 5 ? fil >  pageList.length - 11 : (fil > pg -5 && fil < pg + 5) ) : fil < 10 )
                                .map(item => <span className={ F_styles.page_box } style={{ backgroundColor: item === pg ? 'cadetblue' : 'white', color : item === pg ? 'white' : 'black'}} 

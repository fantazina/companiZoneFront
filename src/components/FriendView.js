...
const FriendView = ({onPage, F_styles ,seq, mainPage}) => {
    ...
    return (
        <div>
            <div className={ F_styles.view_container }>
                { writeDTO && 
                    <div className={ F_styles.view_main }>
                        <div className={ F_styles.top_div }>
                            <div className={ F_styles.back }>
                                <button onClick={ () => onChangePage(0) }>뒤로</button>
                            </div>

                            <h2 className={ F_styles.view_title }>[{ writeDTO.sel }]{ writeDTO.title }</h2> 
                                { writeDTO && window.sessionStorage.getItem('userDTO') && JSON.parse(window.sessionStorage.getItem('userDTO')).id === writeDTO.id && (
                                    <div className={ F_styles.up_button }>
                                        <button onClick={ () => onPage(3) }>수정</button>
                                        <button onClick={ onDeleteSubmit }>삭제</button>
                                    </div>
                                )}
                            <div className={ F_styles.id_list }>{ writeDTO.id } | { getToday(writeDTO.logTime) } | { writeDTO.hit }</div>
                        </div>

                        <div className={ F_styles.view_content }>
                            <div className={ F_styles.view_text }>{ writeDTO.content }</div>
                        </div>

                        <div className={ F_styles.view_img}>
                            {
            ...
            { modal && <div>
                <div className={ F_styles.bgImg } onClick={ () => setModal(false)}/>
                <img className={ F_styles.onImg } src={ modalImg } alt='사진'/>
            </div>
            }

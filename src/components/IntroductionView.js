...
const IntroductionView = ({onPage, I_styles ,seq, mainPage}) => {
    ...
    return (
        <div>
            <div className={ I_styles.view_container }>
                {writeDTO && 
                    <div className={ I_styles.view_main }>
                        <div className={ I_styles.top_div }>
                            <div className={ I_styles.back }>
                                <button onClick={ () => onChangePage(0) }>뒤로</button>
                            </div>

                            <h2 className={ I_styles.view_title }>[{ writeDTO.sel }]{ writeDTO.title }</h2> 
                                { writeDTO && window.sessionStorage.getItem('userDTO') && JSON.parse(window.sessionStorage.getItem('userDTO')).id === writeDTO.id && (
                                    <div className={ I_styles.up_button }>
                                        <button onClick={ () => onPage(3) }>수정</button>
                                        <button onClick={ onDeleteSubmit }>삭제</button>
                                    </div>
                                )}
                            <div className={ I_styles.id_list }>{ writeDTO.id } | { getToday(writeDTO.logTime) } | { writeDTO.hit }</div>
                        </div>

                        <div className={ I_styles.view_content }>
                            <div className={ I_styles.view_text }>{ writeDTO.content }</div>
                        </div>

                        <div className={ I_styles.view_img}>
                            {
            ...
            {modal && <div>
                <div className={ I_styles.bgImg } onClick={ () => setModal(false)}/>
                <img className={ I_styles.onImg } src={ modalImg } alt='사진'/>
            </div>
            }

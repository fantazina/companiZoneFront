...
const PresentList = ({ onPage, onSeq, P_styles, pg, onPg, mainOnPage }) => {
    ...
    return (
        <div>
            <div className={ P_styles.upload_main }>
                <h2 style={{ display : 'inline-block' }}>물품 나눔 게시판</h2>
                    <div className={ P_styles.search }>
                    ...
                    <div className={P_styles.presentList}>
                        <div style={{ borderTop : '1px solid gray',borderBottom : '1px solid gray', padding : '5px'}}>
                            ...
                            <span className={ P_styles.logTime }>작성일자</span>
                            <span className={ P_styles.hit }>조회수</span>
                        </div>
                    ...
                    <div>
                        <div className={ P_styles.page_num }>{pg > 4  && <span onClick={() => onPg(0 > pg - 10 ? 0 : pg - 10)}>이전</span>}
                            { pageList.filter(fil => pg > 4 ? ( pageList.length - 1 < pg + 5 ? fil >  pageList.length - 11 : (fil > pg -5 && fil < pg + 5) ) : fil < 10 )
                                .map(item => <span className={ P_styles.page_box } style={{ backgroundColor: item === pg ? 'cadetblue' : 'white', color : item === pg ? 'white' : 'black' }} 

...
const PresentView = ({onPage, P_styles ,seq, mainPage}) => {
    ...
    return (
        <div>
            <div className={ P_styles.view_container }>
                { writeDTO && 
                    <div className={ P_styles.view_main }>
                        <div className={ P_styles.top_div }>
                            <div className={ P_styles.back }>
                                <button onClick={ () => onChangePage(0) }>뒤로</button>
                            </div>

                            <h2 className={ P_styles.view_title }>[{ writeDTO.sel }]{ writeDTO.title }</h2> 
                                { writeDTO && window.sessionStorage.getItem('userDTO') && JSON.parse(window.sessionStorage.getItem('userDTO')).id === writeDTO.id && (
                                    <div className={ P_styles.up_button }>
                                        <button onClick={ () => onPage(3) }>수정</button>
                                        <button onClick={ onDeleteSubmit }>삭제</button>
                                    </div>
                                )}
                            <div className={ P_styles.id_list }>{ writeDTO.id } | { getToday(writeDTO.logTime) } | { writeDTO.hit }</div>
                        </div>

                        <div className={ P_styles.view_content }>
                            <div className={ P_styles.view_text }>{ writeDTO.content }</div>
                        </div>

                        <div className={ P_styles.view_img}>
                            {
            ...
            { modal && <div>
                <div className={ P_styles.bgImg } onClick={ () => setModal(false)}/>
                <img className={ P_styles.onImg } src={ modalImg } alt='사진'/>
            </div>
            }
